...
| Web app | `app.py` | the classifier's learning step (`/confirm`) and the static files — it answers nothing about a board, and reads no screenshot anything still asks it to read. `/parse`, `/killer/parse` and `/share/parse` are still served but no longer called by anything, and go when this app does |
