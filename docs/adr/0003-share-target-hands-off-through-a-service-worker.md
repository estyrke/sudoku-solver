...
- Screenshot reading stays on the network, because the CV reader is still server-side. That makes it the one path a lost connection takes away, so it must say so in words rather than surfacing `fetch`'s "Failed to fetch" — see `web/ui/offline.ts`.
